      ...
      <table>
        ...
        <tbody>
          <tr v-for="proposal in filteredProposals" :key="proposal.id">
            ...
            <td>{{ proposal.updatedAt?.substring(0,10) }}</td>
            <td>
              <button class="view" @click="goToReview(proposal.id)">View</button>
            </td>
          </tr>
          <tr v-if="filteredProposals.length===0">
            <td colspan="6" style="text-align:center">No proposals found.</td>
          </tr>
        </tbody>
      </table>
    </div>
...
const loadEndorsedProposals = async () => {
  try {
    // Show proposals where REC approved and OVCRIGE needs to forward to OC
    const response = await api.get('/api/proposals?statusIn=REC_APPROVED&statusIn=OVC_APPROVED&statusIn=FOR_OC_APPROVAL')
...
const goToReview = (id) => {
  router.push({ name: 'OvcReviewProposal', params: { id } })
}
</script>
