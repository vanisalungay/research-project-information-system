...
<div class="card">
  ...
  <div class="info-grid">
    ...
    <div v-if="error" class="error-state">
      <p>{{ error }}</p>
      <button class="btn-primary" @click="goToSubmission">Go Back</button>
    </div>

    <!-- REJECT CONFIRMATION MODAL -->
    <div v-if="showRejectModal && !error" class="modal-overlay">
      <div class="modal-box">
  ...
  if (proposalId) {
    try {
      ...
      proposal.value = {
        ...
        programId: data.programTitle || '',
        category: data.programTitle || ''
      }
    } catch (err) {
      console.error(err)
